...
public class main {
  ...
  private static void compile( ArrayList<File> fileList ) {
    ...
    for (File file: fileList) {
      outputFile = new File(file.getPath().split(".jack")[0] + ".vm");
      System.out.println(Arrays.toString(file.getAbsolutePath().split(".jack")) + ".vm");

      analyzer = new CompilationEngine(file, outputFile);

      analyzer.compileClass();
      closeFile( outputFile.toString(), outputFile );
      }
    }
